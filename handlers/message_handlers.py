...
from database.db_utils import get_data_db, update_user_db, get_user_sul
...
def setup_message_handlers(bot: TeleBot):
    """
...
def handle_contact(message):
    ...
    if user_data:
        ...
        bot.edit_message_text(chat_id=send_message.chat.id, message_id=send_message.message_id,
...
def callback_handelr(call):
    user_id = call.from_user.id
    if call.data == 'noregister':
        if user_id in time_cash_user:
            del time_cash_user[user_id]
        bot.answer_callback_query(call.id, read_template('101'), reply_markup(1))
    elif call.data == 'register':
        bot.answer_callback_query(call.id, read_template('104'))

        user_sul = get_user_sul(phone)
        if userl_sul.get('code') == 0:
